...
class LearnWithQuiz(QMainWindow):
    def __init__(self):
        ...
        self.load_words()
        print("Before loading extended words")
        self.load_extended_words()
        print("After loading extended words")
    ...
    def load_extended_words(self):
        print("Loading extended words...")
        data = self.data_manager.load_json('learnwithquiz_words_extended.json')
        self.extended_words = data.get('words', [])
        print(f"Loaded {len(self.extended_words)} words")

        if not self.extended_words:
            print("Extended words list is empty. Initializing with default values.")
            self.extended_words = [
                {
                    "english": "example",
                    "turkish": ["örnek"],
        ...
        self.data_manager.save_json('learnwithquiz_words_extended.json', {'words': self.extended_words})
        print(f"Saved {len(self.extended_words)} extended words")
    ...
    def create_word_groups(self, group_size=50):
        ...
        print(f"Created {len(self.word_groups)} word groups")
    ...
    def show_word_study(self):
        ...
        # extended_words'ün boş olup olmadığını kontrol et
        if not self.extended_words:
            QMessageBox.warning(self, "Hata", "Kelime listesi boş. Lütfen önce kelime ekleyin.")
            return
        
        self.create_word_groups()
        self.update_group_selector()
        
        # word_groups'un boş olup olmadığını kontrol et
        if not self.word_groups:
            QMessageBox.warning(self, "Hata", "Kelime grupları oluşturulamadı.")
            return
    ...
    def start_word_study(self):
        if self.extended_words:
            ...
            QMessageBox.warning(self, "Hata", "Kelime dosyası yüklenemedi. Lütfen learnwithquiz_words_extended.json dosyasını kontrol edin.")
    ...
    def add_word(self):
        ...
        if ok and english:
            ...
            if ok and turkish:
                turkish_list = [t.strip() for t in turkish.split(',')]
                
                # Extended words için yeni kelime
                new_extended_word = {
                    "english": english,
                    "turkish": turkish_list,
                    "sentences": []
                }
                ...
                if add_sentence == QMessageBox.Yes:
                    ...
                    if ok and english_sentence:
                        ...
                        if ok and turkish_sentence:
                            new_extended_word["sentences"].append({
                                'english': english_sentence,
                                'turkish': turkish_sentence
                            })

                # Extended words'e ekleme
                self.extended_words.append(new_extended_word)
                self.data_manager.save_json('learnwithquiz_words_extended.json', {'words': self.extended_words})

                # Normal words'e ekleme
                self.words[english] = turkish_list
                self.data_manager.save_json('learnwithquiz_words.json', self.words)

                QMessageBox.information(self, 'Kelime Eklendi', 'Yeni kelime başarıyla eklendi.')
                self.load_extended_words()  # Kelime listesini güncelle
                self.create_word_groups()  # Kelime gruplarını yeniden oluştur
                self.load_words()  # Normal kelime listesini güncelle
