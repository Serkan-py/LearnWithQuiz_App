...
class LearnWithQuiz(QMainWindow):
    def __init__(self):
        ...
        self.settings = QSettings('LearnWithQuiz', 'AppSettings')
        self.load_words()
        self.load_mistake_words()  # Hata yapılan kelimeleri yükle
        self.load_theme()
    ...
    def load_extended_words(self):
        try:
            with open(os.path.join(self.base_path, 'learnwithquiz_words_extended.json'), 'r', encoding='utf-8') as f:
                data = json.load(f)
                self.extended_words = data['words']
        except FileNotFoundError:
            self.extended_words = []
            QMessageBox.warning(self, "Hata", "yokdil_fen_words_extended.json dosyası bulunamadı. Lütfen dosyayı oluşturun.")
        except json.JSONDecodeError:
            self.extended_words = []
            QMessageBox.warning(self, "Hata", "yokdil_fen_words_extended.json dosyası geçersiz JSON formatında. Lütfen dosyayı kontrol edin.")
    ...
    def initUI(self):
        self.setWindowTitle('LearnWithQuiz: Dil Öğrenme Asistanı')
    ...
    def create_main_menu(self):
        ...
        title_label = QLabel('LearnWithQuiz')
    ...
    def load_sounds(self):
        ...
        correct_sound_path = os.path.join(base_path, 'sounds', 'correct.wav')
        incorrect_sound_path = os.path.join(base_path, 'sounds', 'incorrect.wav')
    ...
    def load_words(self):
        try:
            with open(os.path.join(self.base_path, 'learnwithquiz_words.json'), 'r', encoding='utf-8') as f:
    ...
    def save_words(self):
        with open(os.path.join(self.base_path, 'learnwithquiz_words.json'), 'w', encoding='utf-8') as f:
    ...
    def load_mistake_words(self):
        try:
            with open(os.path.join(self.base_path, 'learnwithquiz_mistake_words.json'), 'r', encoding='utf-8') as f:
                self.mistake_words = json.load(f)
        except FileNotFoundError:
            self.mistake_words = {}
    ...
    def save_mistake_words(self):
        with open(os.path.join(self.base_path, 'learnwithquiz_mistake_words.json'), 'w', encoding='utf-8') as f:
            json.dump(self.mistake_words, f, ensure_ascii=False, indent=4)
